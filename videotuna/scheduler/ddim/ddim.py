...
class DDIMSampler(object):
    ...
    def make_schedule(
        ...
        self.register_buffer("betas", to_torch(self.model.scheduler.betas))
        self.register_buffer("alphas_cumprod", to_torch(alphas_cumprod))
        self.register_buffer(
            "alphas_cumprod_prev",
            to_torch(self.model.scheduler.alphas_cumprod_prev),
        )
        ...
        for i, step in enumerate(iterator):
            ...
            if start_timesteps is not None:
                ...
                if step > start_timesteps * time_range[0]:
                    ...
                    img = self.model.scheduler.q_sample(x0, ts)
                    init_x0 = True
            ...
            if mask is not None:
                ...
                if clean_cond:  # blend using x0_known, rather than xt_known
                    ...
                    img_orig = self.model.scheduler.q_sample(
                        x0, ts
        ...
        **kwargs,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.0:
            ...
            if guidance_rescale > 0.0:
                e_t = rescale_noise_cfg(
                    e_t, e_t_cond, guidance_rescale=guidance_rescale
                )
            model_output = e_t

        if self.model.parameterization == "v":
            e_t = self.model.scheduler.predict_eps_from_z_and_v(x, t, e_t)
        ...
        alphas_prev = (
            self.model.scheduler.alphas_cumprod_prev
            if use_original_steps
            else self.ddim_alphas_prev
        )
        sqrt_one_minus_alphas = (
            self.model.scheduler.sqrt_one_minus_alphas_cumprod
            if use_original_steps
        ...
        if self.model.parameterization != "v":
            ...
            pred_x0 = self.model.scheduler.predict_start_from_z_and_v(
                x, t, model_output
            )
