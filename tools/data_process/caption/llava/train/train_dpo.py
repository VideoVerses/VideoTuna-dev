...
def preprocess_multimodal(sources: Sequence[str], data_args: DataArguments) -> Dict:
    ...
    for source in sources:
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence["value"] and not sentence[
                "value"
            ].startswith(DEFAULT_IMAGE_TOKEN):
                sentence["value"] = (
        ...
        for j, sentence in enumerate(source):
            ...
            if has_image and "<image>" in sentence["value"]:
                ...
                _input_id = (
                    ...
                    + [IMAGE_TOKEN_INDEX]
                    + nl_tokens
                    + tokenizer(sentence["value"][len("<image>") :]).input_ids
                    + [im_end]
                    + nl_tokens
                )
            else:
...
def preprocess_llama3(
    ...
    for i, source in enumerate(sources):
        ...
        input_id, target = [], []
        system = (
            tokenizer("<|begin_of_text|>").input_ids
            + tokenizer("<|start_header_id|>system<|end_header_id|>").input_ids
            + nl_tokens * 2
            + tokenizer(system_message).input_ids
            + [eot_id]
        )
        ...
        for j, sentence in enumerate(source):
            ...
            if role == "<|start_header_id|>user<|end_header_id|>":
                ...
                _target = (
                    [IGNORE_INDEX] * (len(tokenizer(role).input_ids) + 2)
                    + _input_id[len(tokenizer(role).input_ids) + 2 : -1]
                    + [eot_id]
                )
            else:
                raise NotImplementedError
...
class DPODataset(Dataset):
    ...
    def __init__(
        ...
        if "{" in data_path and "}" in data_path:
            base_path, file_pattern = re.match(
                r"^(.*)\{(.*)\}\.json$", data_path
            ).groups()
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            cur_len = (
                len(sample["prompt"].split())
                + len(sample["answer"].split())
                + len(sample["chosen"].split())
                + len(sample["rejected"].split())
            )
    ...
    def process_image(self, image_file):
        ...
        if self.data_args.image_aspect_ratio == "highres":
            ...
            self.data_args.image_aspect_ratio == "anyres"
            or "anyres" in self.data_args.image_aspect_ratio
        ):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            "video" in sources[0]
        ):  # FIXME: This logic should be largely improved by Yuanhan. It's too messy now.
        ...
        )  # inplace modification following
...
class DPODataCollator(DPODataCollatorWithPadding):
    ...
    def tokenize_batch_element(
        self, prompt: str, chosen: str, rejected: str, has_image: bool = True
    ) -> Dict:
        ...
        rejected_data_dict = preprocess(
            [rejected_sources], self.tokenizer, has_image=has_image
        )
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            ...
            batch_element = self.tokenize_batch_element(
                prompt, chosen, rejected, has_image=has_image
            )
        ...
        if "image" in features[0]:
            ...
            padded_batch["image_sizes"] = [
                im[1] for im_list in images for im in im_list
            ]
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if "qwen" in model_args.model_name_or_path.lower():
        ...
        cfg_pretrained = LlavaMistralConfig.from_pretrained(
    ...
    if (
        ...
        and cfg_pretrained is not None
    ):
    ...
    if model_args.model_class_name is not None:
        ...
        rank0_print(
            f"Using model class {model_class} from {model_args.model_class_name}"
        )
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            or "quyen" in model_args.model_name_or_path.lower()
        ):
...
def train(attn_implementation=None):
    ...
    if training_args.gradient_checkpointing:
        if hasattr(model, "enable_input_require_grads"):
            ...
            if ref_model is not None:
                ref_model.get_input_embeddings().register_forward_hook(
    ...
    if model_args.vision_tower is not None:
        ...
        if data_args.image_grid_pinpoints is not None:
            ...
            if (
                "x" in data_args.image_grid_pinpoints
                and "..." in data_args.image_grid_pinpoints
            ):
                ...
                grid_pinpoints = [
                    [dim * vis_encoder_size for dim in pair] for pair in grid_pinpoints
                ]
                data_args.image_grid_pinpoints = grid_pinpoints
            elif "x" in data_args.image_grid_pinpoints:
                vis_encoder_size = data_args.image_processor.size[0]
                assert vis_encoder_size in [
                    224,
                    336,
                    384,
                    448,
                    512,
                ], "vis_encoder_size should be in [224, 336, 384, 448, 512]"
                grid_pinpoints = (
                    data_args.image_grid_pinpoints.replace(" ", "")
                    .replace("x", ",")[1:-1]
                    .split("),(")
                )
                data_args.image_grid_pinpoints = [
                    [int(x) * vis_encoder_size for x in item.split(",")]
                    for item in grid_pinpoints
                ]
            else:
